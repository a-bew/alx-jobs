      ...
      passport.authenticate(
        ...
        async (err: Error, user: User, info: { message: string }) => {
          ...
          if (foundUser && !foundUser.emailVerified) {
            return reject(createHttpError(403, 'Email not verified. Please verify your email before logging in.'));
          }
...
const loginController = asyncWrapper(async (req: Request|any, res: Response, next: NextFunction) => {
  try {
    ...
    return sendResponse(res, { token: accessToken, role: [user.role] }, 201);
